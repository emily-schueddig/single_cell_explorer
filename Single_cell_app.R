        ...
        mainPanel(
          ...
          tabsetPanel(
            ...
            tabPanel(
              ...
              uiOutput("violin_color_ui"),
              textInput("gene_violin_title", "Plot Title", value = "Gene Expression Violin Plot"),
              textInput("gene_violin_xlab", "X-axis Label", value = "Group"),
              textInput("gene_violin_ylab", "Y-axis Label", value = "Expression"),
            ...
            tabPanel(
              ...
              uiOutput("filtered_violin_color_ui"),
              textInput("filtered_violin_title", "Plot Title", value = "Filtered Gene Expression Violin Plot"),
              textInput("filtered_violin_xlab", "X-axis Label", value = "Group"),
              textInput("filtered_violin_ylab", "Y-axis Label", value = "Expression"),
...
server <- function(input, output, session) {
  ...
  output$violin_color_ui <- renderUI({
    req(seurat_obj(), input$violin_group)
  ...
  # ---- Dynamic color pickers for violin groups ----
  ...
  # ---- Dynamic color pickers for FILTERED violin groups ----
  output$filtered_violin_color_ui <- renderUI({
    req(seurat_obj(), input$filtered_violin_group)
  ...
  output$download_gene_violin <- downloadHandler(
    ...
    content = function(file) {
      ggsave(file, plot = get_last_plot("gene_violin_raw"),
 ...
 #summary table
  ...
  output$download_filtered_violins <- downloadHandler(
    filename = function() sprintf("filtered_gene_violins_%s.png", Sys.Date()),
    content = function(file) {
      ggsave(file, plot = get_last_plot("filtered_violin_raw"),
             width = 8, height = 6, dpi = 300)
    }
  )
